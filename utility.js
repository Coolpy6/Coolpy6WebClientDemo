...
function onMessageArrived(message) {
  ...
  var decoded = msgpack.decode(message.payloadBytes);
  row.insertCell(1).innerHTML = JSON.stringify(decoded);
  ...
  if(!document.getElementById(message.destinationName)){
      ...
      newlastMessageRow.insertCell(1).innerHTML = JSON.stringify(decoded);
      ...
      lastMessageRow.cells[1].innerHTML = JSON.stringify(decoded);
...
function publish(){
    ...
    var encoded = msgpack.encode(JSON.parse(message));
    message = new Paho.MQTT.Message(encoded);
